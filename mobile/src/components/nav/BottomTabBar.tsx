...
interface Tab {
  /** Typed expo-router path. `Href` narrows to the union of valid routes
   *  generated by `typedRoutes: true` so wrong paths fail at compile time. */
  href: Href;
  label: string;
  Icon: LucideIcon;
}
...
function isActive(pathname: string, href: Href): boolean {
  // `Href` can be a string OR a `{ pathname, params }` object — for the
  // tab bar we only ever pass string literals, so coerce.
  ...
  const tail = hrefStr.replace("/(tabs)", "");
  ...
  return (
    ...
    >
      {TABS.map((t) => {
        ...
        return (
          <Pressable
            key={typeof t.href === "string" ? t.href : t.href.pathname}
            onPress={() => {
              // Only fire haptic when actually switching tabs — repeated
              // taps on the active tab shouldn't buzz.
