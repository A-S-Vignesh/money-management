...
export default function PricingPage() {
  const plans = [
    ...
    },
    {
      ...
      icon: <Building className="h-8 w-8 text-purple-600 dark:text-purple-300" />,
    },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="flex justify-center mb-12">
        <div className="inline-flex rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-1">
          ...
          <button className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 rounded-md hover:text-indigo-600">
            Annual (Save 20%)
          </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {plans.map((plan, index) => (
            ...
            >
              ...
              <div
                className={`p-8 ${
                  plan.popular ? "bg-indigo-50 dark:bg-indigo-950/40" : ""
                } rounded-t-2xl`}
              >
                <div className="flex items-center mb-4">
                  {plan.icon}
                  <h3 className="ml-3 text-2xl font-bold text-gray-900 dark:text-gray-100">
                    {plan.name}
                  </h3>
                </div>
          ...
          {faqs.map((faq, index) => (
            <div key={index} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-6">
              <div className="flex items-start">
                <HelpCircle className="h-6 w-6 text-indigo-600 dark:text-indigo-300 mt-0.5 flex-shrink-0" />
                <div className="ml-4">
